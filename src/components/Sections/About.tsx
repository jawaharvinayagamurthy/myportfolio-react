...
const About: FC = memo(() => {
  ...
  return (
    <Section className="bg-neutral-500 rounded-3xl" sectionId={SectionId.About}>
      <div className={classNames('grid grid-cols-1', {'md:grid-cols-4': !!profileImageSrc})}>
        {!!profileImageSrc && (
          ...
          <ul className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {aboutItems.map(({label, text, Icon}, idx) => (
              <li className="col-span-1 flex  items-start gap-x-2" key={idx}>
                ...
                <span className="text-sm font text-white">{label}:</span>
                <span className=" text-sm text-white mb-0">{text}</span>
              </li>
            ))}
          </ul>
        </div>
